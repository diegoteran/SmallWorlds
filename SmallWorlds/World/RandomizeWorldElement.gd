...
func _on_VisibilityArea_body_entered(_body):
	tween.interpolate_property(sprite, "modulate", sprite.modulate, Color(1, 1, 1, 0.4), 0.6, Tween.TRANS_LINEAR, Tween.EASE_IN)
	tween.start()

func _on_VisibilityArea_body_exited(_body):
	tween.interpolate_property(sprite, "modulate", sprite.modulate, Color(1, 1, 1, 1), 0.6, Tween.TRANS_LINEAR, Tween.EASE_IN)
	tween.start()
