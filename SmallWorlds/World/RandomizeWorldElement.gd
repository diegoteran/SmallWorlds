...
func _on_VisibilityArea_body_entered(_body):
	tween.interpolate_property(sprite, "modulate:a", sprite.modulate.a, 0.2, 0.3, Tween.TRANS_LINEAR, Tween.EASE_IN)
	tween.start()

func _on_VisibilityArea_body_exited(_body):
	tween.interpolate_property(sprite, "modulate:a", sprite.modulate.a, 1, 0.3, Tween.TRANS_LINEAR, Tween.EASE_IN)
	tween.start()
